...
export function OrganizationalClock({ items }: OrganizationalClockProps) {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Pin the clock section
      ScrollTrigger.create({
        trigger: containerRef.current,
        start: 'top top',
        end: 'bottom bottom',
        pin: clockRef.current,
        pinSpacing: true,
      });
      ...
      sections.forEach((section, index) => {
        gsap.to(hand, {
          scrollTrigger: {
            ...
            onToggle: (self) => {
                if (self.isActive) {
                    gsap.to(clockItems[index], { scale: 1.2, color: 'hsl(var(--primary))', duration: 0.3 });
                } else {
                    gsap.to(clockItems[index], { scale: 1, color: 'hsl(var(--muted-foreground))', duration: 0.3 });
                }
            }
          },
  ...
  return (
    <div ref={containerRef} className="lg:grid lg:grid-cols-2 lg:gap-24 items-start">
      ...
      {/* Right Column: Clock */}
      <div ref={clockRef} className="hidden lg:flex items-center justify-center h-screen sticky top-0">
